  ...
  private Router setupRouter() {
    ...
    .handler(ctx -> {
      selectQuotes().compose(rows -> {
        ...
        return ctx.response()
          .putHeader(HttpHeaderNames.CONTENT_TYPE, CONTENT_TYPE_JSON)
    ...
    .produces(CONTENT_TYPE_JSON)
    .handler(BodyHandler.create(false).setBodyLimit(100 * 1024))
    .handler(ctx -> {
      ...
      insertQuoteAndReturn(jsonObject.getString("author", "Unknown"), jsonObject.getString("text"))
      .compose(rows -> {
    ...
    router.get("/realtime").handler(ctx -> {
      ctx.request()
        .toWebSocket()
        .onSuccess(webSocket -> {
          MessageConsumer<JsonObject> consumer = ctx.vertx()
            .eventBus()
